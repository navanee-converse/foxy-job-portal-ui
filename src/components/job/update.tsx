...
import { DynamicListSection } from "../field/dynamic-list-selection";
import { JobDescriptionField } from "../field/job-description";
...
const UpdateJob = () => {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        const response: Job = await request(`/jobs/${id}`, "GET");
        const jobData = response;

        const flattenedTags =
          jobData.jobTags?.map((item) => ({
            _id: item.tagId._id,
  ...
  const handleJobUpdate = async (
    ...
    try {
      const tagIdsOnly = values.tagIds.map((tag) =>
