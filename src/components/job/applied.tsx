...
const AppliedJobs = () => {
  ...
  const handleViewResume = (url: string) => {
    if (url.endsWith(".pdf")) {
      ...
      const encodedResumeUrl = encodeURIComponent(url);
      window.open(
          ...
          {isLoading ? (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data.map((app) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-5">
                          <Badge
                            className={`capitalize px-3 py-1 font-medium border shadow-none ${getStatusColor(app.status)}`}
                          >
                            {app.status}
                          </Badge>
                        </td>
                ...
                {data.map((app) => (
                  <div key={app._id} className="p-5 space-y-4">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge
                        className={`capitalize text-[10px] px-2 py-0 border shadow-none ${getStatusColor(app.status)}`}
                      >
                        {app.status}
                      </Badge>
                    </div>
            ...
            <div className="flex flex-col items-center justify-centergrow py-20 px-4 text-center">
              ...
              <p className="text-slate-500 max-w-sm mx-auto mt-2">
                It looks like you haven't applied to any jobs yet. Start your
                journey by browsing available openings.
              </p>
              <Button
          ...
          {pagination.totalPages > 1 && (
            <div className="mt-auto px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row gap-4 items-center justify-between">
              <p className="text-sm text-slate-600 font-medium order-2 sm:order-1">
                Page <span className="text-slate-900">{pagination.page}</span>{" "}
                of{" "}
